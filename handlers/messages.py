    ...
    try:
        ...
        if websearch_status:
            answer = await ChatGPT(
                user_id=user_id, user_text=message.text, model=model # type: ignore
...
async def handle_photo(message: Message, bot: Bot) -> None:
    try:
        photo = await bot.get_file(message.photo[-1].file_id) # type: ignore
        url = f"https://api.telegram.org/file/bot{BOT_TOKEN}/{photo.file_path}"
        answer = await ChatGPT(
            user_id=message.from_user.id, user_text=message.caption # type: ignore
        ).answer_on_photo(url)
        if len(answer) <= 4096:
